    ...
    return (
        <>
            <section className='m-2 my-10 sm:mx-20'>
                ...
                <div className='min-h-[80vh] mt-4 overflow-hidden grid grid-cols-1 md:grid-cols-2 gap-6 '>
                    {/* Big image */}
                    <Link to={`/recipes/recipe/${heroData._id}`} className="sideLeft hover:scale-[1.03] duration-200 relative h-full w-full row-span-2">
                        <img src={heroData.image} alt={heroData.title} className='w-full h-full object-cover' />
                        <h1 className='text-white w-full text-3xl text-center font-bold textShadow absolute bottom-2 md:bottom-20'>{heroData.title}</h1>
                    </Link>
                    {/* Smaller images */}
                    <div className="sideRight h-full w-full grid grid-cols-2 grid-rows-2 gap-6">
                        {restData.map((val) =>
                            <Link to={`/recipes/recipe/${val._id}`} key={val._id} className='relative hover:scale-[1.03] duration-200'>
                                <img src={val.image} alt={val.title} className='w-full h-72 object-cover border-2W border-black' />
